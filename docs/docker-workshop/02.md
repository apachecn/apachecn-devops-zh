...
这将使用名为`Dockerfile`的文本文件来完成。该文件由 Docker 可以执行的命令组成，以创建 Docker 映像。使用`docker build`(或`docker image build`)命令从`Dockerfile`创建Docker映像。
...
一个`Dockerfile`可以包含多行注释和指令。这些行将由**Docker引擎**在构建Docker映像时按顺序执行。像编程语言一样，一个`Dockerfile`也可以包含注释。
...
在上一节中，我们学习了如何创建`Dockerfile`。该过程的下一步是使用`Dockerfile`构建**Docker映像**。
