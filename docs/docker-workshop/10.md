...
Kubernetes 在希腊语中翻译为**船长**。用Docker对船只和容器的比喻，Kubernetes把自己定位为航海大师。Kubernetes 的想法源于管理谷歌服务(如 Gmail 或谷歌驱动)的容器已有十多年。从 2014 年至今，Kubernetes 一直是开源项目，由**云原生计算基金会** ( **CNCF** )管理。
