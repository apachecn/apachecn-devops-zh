...
如果你想在不被技术细节淹没的情况下利用它们，把Docker研讨会添加到你的阅读列表中！
...
*第 7 章*、*Docker仓库*，详细说明了Docker容器和环境中的仓库利用率。

*第 8 章*、 *CI/CD 管道*描述了使用 Jenkins 创建持续集成/持续部署管道。

*第 9 章**DockerSwarm*，介绍了使用群来编排Docker服务。
...
**Docker中心帐户**:您可以在 [https://hub.docker.com/](https://hub.docker.com/) 上创建自由Docker帐户。
