...
7.  Use the `docker run` command to start an instance of the Ubuntu 18.04 container:
    ...
    请注意，这次我们使用带有`-d`标志的`docker run`命令。这告诉 Docker 在守护模式下(或在后台)运行容器。如果我们忽略`-d`标志，容器将接管我们当前的Docker，直到容器内的主要流程终止。
...
官方 Postgres 容器映像提供了许多环境变量覆盖，您可以利用它们来配置 PostgreSQL 实例。查看位于 https://hub.docker.com/_/postgres Docker中心的容器文件。
