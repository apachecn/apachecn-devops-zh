...
在我们的集装箱之旅中，我们将经常从本地环境中拉出、启动、停止和移除集装箱。在生产环境中部署容器之前，我们首先要在本地运行容器，以了解它是如何工作的以及正常行为是什么样子，这一点非常重要。这包括启动容器、停止容器、获取关于容器如何运行的详细信息，当然，还包括访问容器日志以查看关于容器内运行的应用的关键细节。这些基本命令概述如下:
