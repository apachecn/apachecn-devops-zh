...
6.  在`builder-pattern`目录下，创建一个名为`Dockerfile.build`的文件。该文件将包含所有的指令，您将使用这些指令来创建`build`码头工人映像:
...
8.  Add the following content to the `Dockerfile.build` file and save the file:
    ...
    这与您在*练习 4.01**中为`Dockerfile`创建的内容相同，使用正常构建过程*构建码头工人映像。
...
在*练习 4.02* 、*使用构建器图案*构建码头工人映像时，您使用了构建器图案来优化码头工人映像的大小。然而，有一个操作负担，因为您需要在 Docker 映像构建过程中管理两个`Dockerfiles`和一个 shell 脚本。在本练习中，您将使用多阶段`Dockerfile`来消除这一操作负担。
