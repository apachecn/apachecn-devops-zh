...
在 Docker 之旅的早期，您可能会使用私有注册表。当您开始整理现有的应用并尝试新的软件栈时，在快速的本地网络中推送和拉入映像可能会很有用，如果本地存储空间有问题，也可以使用云。随着您越来越多地使用 Docker 并推进到生产实施，您可能会有一个升级到 DTR 的路线图，以获得具有丰富安全功能的受支持的注册表。
