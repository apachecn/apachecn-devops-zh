...
在[第 8 章](08.html)*管理和监控Docker化解决方案*中，我演示了Docker企业中的**通用控制平面**(**UCP**)**容器即服务** ( **CaaS** )平台。启动和管理 Docker 容器的标准 API 让这个工具提供了一个统一的管理和行政体验。Docker 平台的开放性让开源工具可以采用相同的方法来进行丰富、整合的监控。
