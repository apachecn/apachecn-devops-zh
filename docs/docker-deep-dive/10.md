...
在本节中，我们将构建一个安全的Swarm集群，其中有三个*管理节点*和三个*工作节点*。您可以使用不同数量的*经理*和*工人*以及不同名称和 IPs 的不同实验室，但是下面的示例将使用图 10.2 中的值。
...
Mac 和 Windows 的 Docker 桌面仅支持单个 Docker 节点。您可以初始化一个单节点集群，并遵循大多数示例。或者，你可以试试在 https://labs.play-with-docker.com 玩Docker。
