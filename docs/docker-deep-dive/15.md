...
**Docker群体模式**默认是安全的。您无需配置即可获得以下所有内容；加密节点 id、相互身份验证、自动 CA 配置、自动证书轮换、加密集群存储、加密网络等等。
...
许多应用需要秘密——比如密码、TLS 证书、SSH 密钥等等。

Docker 的早期版本没有标准化的方法来安全地让应用获得秘密。开发人员通过纯文本环境变量将秘密插入应用是很常见的(我们都做过)。这远非理想。
...
秘密以未加密的形式出现在运行的容器中的原因是，应用可以使用它们，而不需要方法来解密它们。
