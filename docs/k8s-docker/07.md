...
![Figure 7.1 – Kubernetes/kubectl OpenID Connect sequence diagram ](image/Fig_7.1_B15514.jpg)
...
![Figure 7.2 – Authentication architecture ](image/Fig_7.2_B15514.jpg)
...
***   First, we need to need to log into the testing identity provider by going to [https://portal.apps.tremolo.io/](https://portal.apps.tremolo.io/) and clicking on the **SAML2 Test Lab** badge:
    ...
    ![Figure 7.3 – SAML2 Test Lab badge ](image/Fig_7.3_B15514.jpg)
    ...
    *   Once you've clicked on the badge, you'll be presented with a screen that shows your test IdP metadata URL:
    ...
    ![Figure 7.4 – Testing identity provider's page, highlighting the SAML2 metadata URL ](image/Fig_7.4_B15514.jpg)
    ...
    *   现在 OpenUnison 正在运行，我们需要使用`values.yaml`文件中`network.openunison_host`的主机和`/auth/forms/saml2_rp_metadata.jsp`路径
    ...
    ![Figure 7.5 – Testing the identity provider with the relying party metadata  ](image/Fig_7.5_B15514.jpg)
    ...
    *   Finally, we need to add some attributes to our test user. Add the attributes shown in the following screenshot:
    ...
    ![Figure 7.6 – Identity provider test user configuration ](image/Fig_7.6_B15514.jpg)
...
1.  You can retrieve you token from the OpenUnison dashboard. Navigate to the OpenUnison home page and click on the key that says **Kubernetes Tokens**. You'll see a screen that looks as follows:
    ...
    ![Figure 7.10 – OpenUnison kubectl configuration tool ](image/Fig_7.10_B15514.jpg)
...
![Figure 7.11 – Diagram of how a user interacts with the API server when using Impersonation ](image/Fig_7.11_B15514.jpg)
...
![Figure 7.12 – Kubernetes Dashboard with Impersonation ](image/Fig_7.12_B15514.jpg)
