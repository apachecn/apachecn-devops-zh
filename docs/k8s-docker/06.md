...
![Table 6.1: Kubernetes service types ](image/Table_1.jpg)
...
在物理机和虚拟服务器的世界中，你可能已经瞄准了一个 DNS 记录来与服务器进行通信。如果服务器的 IP 地址发生了变化，那么假设您启用了动态 DNS，它不会对应用程序产生任何影响。这就是使用名称而不是 IP 地址作为端点的优势。
...
![Table 6.2: Internal DNS examples ](image/Table_2.jpg)
...
![Figure 6.1 – NGINX service using NodePort ](image/Fig_6.1_B15514.jpg)
...
![Figure 6.2 – NodePort traffic flow overview ](image/Fig_6.2_B15514.jpg)
...
这实现了最初的要求，只使用 Kubernetes DNS 服务器将应用程序连接到外部数据库服务器。
...
![Figure 6.3 – Multiple-name Ingress traffic flow ](image/Fig_6.3_B15514.jpg)
...
![Figure 6.4 – Example name resolution using nip.io ](image/Fig_6.4_B15514.jpg)
...
![Figure 6.5 – Ingress traffic flow ](image/Fig_6.5_B15514.jpg)
...
![Figure 6.6 – NGINX web server using nip.io for Ingress ](image/Fig_6.6_B15514.jpg)
...
4.  To verify that the service was created correctly, list the services using `kubectl get services`:
    ...
    ![Figure 6.7 – Kubectl service output ](image/Fig_6.7_B15514.jpg)
...
![Figure 6.8 – Curl output to the LoadBalancer service running NGINX ](image/Fig_6.8_B15514.jpg)
...
3.  To verify that the service was created with an IP address from the subnet-201 pool, list all of the services:
    ...
    ![Figure 6.9 – Example services using LoadBalancer   ](image/Fig_6.9_B15514.jpg)
...
![Figure 6.10 – Curl NGINX on a LoadBalancer using a second IP pool ](image/Fig_6.10_B15514.jpg)
...
1.  First, look at the services in the `kube-system` namespace:
    ...
    ![Figure 6.11 – Default service list for kube-system ](image/Fig_6.11_B15514.jpg)
 ...
 **![Figure 6.12 – Multiple protocols assigned using MetalLB ](image/Fig_6.12_B15514.jpg)
...
![Figure 6.13 – ETCD operator and nodes ](image/Fig_6.13_B15514.jpg)

图 6.13–ETCD 算子和节点

一旦部署完成，查看`kube-system`名称空间中的服务，以获取名为`etcd-cluster-client`的新 ETCD 服务的 IP 地址:

![Figure 6.14 – ETCD service IP ](image/Fig_6.14_B15514.jpg)
...
6.  To confirm that the entry has been created successfully, execute a `nslookup` for the host in a Netshoot shell:
    ...
    ![Figure 6.15 – Nslookup for new record ](image/Fig_6.15_B15514.jpg)
    ...
    ![Figure 6.16 – NGINX external IP address ](image/Fig_6.16_B15514.jpg)

    图 6.16–NGINX 外部 IP 地址

7.  最后，让我们通过使用名称连接到容器来确认与 NGINX 的连接是否正常。在网络拍摄容器中使用`curl`命令，卷曲到端口 8080 上的域名:

![Figure 6.17 – Curl test using the external-dns name ](image/Fig_6.17_B15514.jpg)
...
![Figure 6.18 – Windows conditional forwarder setup ](image/Fig_6.18_B15514.jpg)
...
![Figure 6.19 – Nslookup confirmation for registered name ](image/Fig_6.19_B15514.jpg)
...
![Figure 6.20 – Success browsing from an external workstation using CoreDNS ](image/Fig_6.20_B15514.jpg)
...
![Figure 6.21 – Nslookup confirmation for an additional registered name ](image/Fig_6.21_B15514.jpg)

图 6.21–Nslookup 确认一个额外的注册名称

为了确认网络服务器运行正常，我们从工作站浏览到以下网址:

![Figure 6.22 – Successful browsing from an external workstation using CoreDNS ](image/Fig_6.22_B15514.jpg)
...
通常，服务本身并不是用于提供对集群中运行的应用程序的访问的唯一对象。您将经常使用集群 IP 服务和入口控制器来提供对使用第 7 层的服务的访问。一些应用程序可能需要额外的通信，这不是由第 7 层负载平衡器提供的。这些应用程序可能需要第 4 层负载平衡器来向用户公开它们的服务。在负载平衡部分，我们演示了 MetalLB 的安装和使用，MetalLB 是一种常用的开源第 7 层负载平衡器。
