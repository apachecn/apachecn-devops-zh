...
![Figure 13.1 – Velero client version output ](image/Fig_13.1_B15514.jpg)
...
![Table 13.1 – Velero storage options ](image/Table_13.1.jpg)
...
![Figure 13.2 – Minio service creation ](image/Fig_13.2_B15514.jpg)
...
2.  Once deployed, you can use a browser on any machine and open the URL you used for the Ingress rule. On our cluster, the host IP is `10.2.1.121`, so our URL is `minio.10.2.1.121.nip.io`:
    ...
    ![Figure 13.3 – MinIO dashboard ](image/Fig_13.3_B15514.jpg)
...
![Figure 13.4 – MinIO Browser ](image/Fig_13.4_B15514.jpg)
...
![Table 13.2 – Velero Install Options ](image/Table_13.2.jpg)
...
![Table 13.3 – Velero's CRDs and Secrets ](image/Table_13.3.jpg)
...
![Figure 13.5 – The Velero describe output ](image/Fig_13.5_B15514.jpg)
...
![Figure 13.6 – The kubectl describe output on the backup resource ](image/Fig_13.6_B15514.jpg)
...
![Figure 13.7 – Cron scheduling expression ](image/Fig_13.7_B15514.jpg)
...
![Table 13.4 – cron shorthand scheduling ](image/Table_13.4.jpg)
...
![](image/Table_13.5a.jpg)

![Table 13.5 – Velero backup flags ](image/Table_13.5b.jpg)
...
![Figure 13.8 – Velero help output ](image/Fig_13.8_B15514.jpg)
...
![Figure 13.9 – The velero get output ](image/Fig_13.9_B15514.jpg)
...
![Figure 13.10 – Velero delete output ](image/Fig_13.10_B15514.jpg)
...
对于第一个例子，我们将使用 NGINX 网络服务器创建一个简单的部署。我们将部署应用程序，验证它是否按预期工作，然后删除部署。使用备份，我们将恢复部署，并通过浏览 web 服务器的主页来测试恢复是否有效。
...
![Figure 13.11 – Verify that NGINX is running ](image/Fig_13.11_B15514.jpg)
...
![Figure 13.12 – Verify whether NGINX is running ](image/Fig_13.12_B15514.jpg)
...
![Figure 13.13 – Verify that NGINX has been restored ](image/Fig_13.13_B15514.jpg)
...
2.  Once the cluster has deployed, verify that your context has been switched to the `velero-restore` cluster:
    ...
    ![Figure 13.14 – Verifying your current context ](image/Fig_13.14_B15514.jpg)
...
![Figure 13.15 – New cluster namespaces ](image/Fig_13.15_B15514.jpg)
...
![Figure 13.16 – Viewing backups on the new cluster ](image/Fig_13.16_B15514.jpg)
...
3.  Once the phase has changed from `InProgress` to `Completed`, verify that your new cluster has the additional demo namespaces using `kubectl get ns`:
    ...
    ![Figure 13.17 – Viewing backups on the new cluster ](image/Fig_13.17_B15514.jpg)
...
![Figure 13.18 – Verifying pods in restored namespaces ](image/Fig_13.18_B15514.jpg)
...
接下来，在我们的最后一章中，我们将汇集本书中的许多以前的课程，为您的开发人员和管理员构建一个平台。我们将添加源代码控制和管道来构建一个平台，允许开发人员构建一个“项目”，签入源代码来创建一个运行的应用程序。
