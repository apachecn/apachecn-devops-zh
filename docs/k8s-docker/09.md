...
![Figure 9.1 – Kubernetes Dashboard architecture ](image/Fig_9.1_B15514.jpg)
...
2.  Look for the `args` option for the container, add `- --enable-skip-login`, then save:
    ...
    ![Figure 9.2 – Enabling skip-login on the dashboard ](image/Fig_9.2_B15514.jpg)
...
6.  You will see an authentication screen that asks for a token or a Kubeconfig file, but since we enabled the option to skip the login when we edited the dashboard, you can simply skip the login by clicking on **Skip**:
    ...
    ![Figure 9.3 – Kubernetes Dashboard with login disabled ](image/Fig_9.3_B15514.jpg)

    图 9.3–禁用登录的 Kubernetes 仪表板

7.  Once in the dashboard, the default service account doesn't have access to anything:

    ![Figure 9.4 – Kubernetes Dashboard with the default service account ](image/Fig_9.4_B15514.jpg)
...
![Figure 9.5 – Kubernetes Dashboard showing malware deployed ](image/Fig_9.5_B15514.jpg)
...
![Figure 9.6 – Kubernetes Dashboard with a reverse proxy ](image/Fig_9.6_B15514.jpg)
...
![Figure 9.7 – Visual Studio Code with the Kubernetes plugin ](image/Fig_9.7_B15514.jpg)
...
![Figure 9.8 – OpenUnison integration with the dashboard ](image/Fig_9.8_B15514.jpg)
