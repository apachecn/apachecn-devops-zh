...
在早期，用户在被这个易于使用的工具迷住后开始使用 Docker，这使他们能够解决许多挑战:拉、打包、隔离，并使应用程序几乎没有负担地跨系统移植。

![Welcome to Docker Swarm](graphics/image_01_001.jpg)
...
使用 Docker Machine，您可以在多个云平台上调配虚拟机和物理机，以及裸机来运行 Docker 容器。使用 Docker Compose，您可以通过用 YAML 简单而强大的语法描述行为来定义 Docker 文件，并通过“组合”这些文件来启动应用程序。Swarm 是一个强大的聚类工具，需要进行更深入的研究。
...
![Swarm goals](graphics/image_01_002.jpg)

# 为什么使用 Swarm

对容器使用集群解决方案有很多原因。随着应用程序的增长，您将面临新的强制性要求，例如可扩展性、可管理性和高可用性。
...
到目前为止，Kubernetes 在 Docker 中运行容器，Docker 是由一个名为 Kubelet 的服务通过 API 命令的，kube let 是一个注册和管理 Pods 的服务。在架构上，Kubernetes 从逻辑上来说，不是将集群划分为裸容器，而是划分为 Pods。Pod 是最小的可部署单元，在物理上是由一个或多个容器组成的应用程序的表示，这些容器通常是并置的，共享存储和网络等资源(用户可以使用 Compose 在 Docker 中模拟 Pod，并从 Docker 1.12 开始创建 Docker **DABs** ( **分布式应用程序捆绑包**)。
...
它甚至比 Fleet 更通用，是多平台的(您可以在 Linux、OS X 或 Windows 节点上运行它)，并且能够运行异构作业。除了纯大数据作业(Hadoop 或 Spark)和其他作业(包括持续集成、实时处理、网络应用程序、数据存储等)之外，您通常可以在 Mesos 上运行容器集群。

一个 Mesos 集群由一个主、从和框架组成。如您所料，主服务器在从服务器上分配资源和任务，负责系统通信并运行发现服务(ZooKeeper)。但是什么是框架呢？框架就是应用程序。一个框架由一个调度器和一个执行器组成，第一个分配任务，第二个执行任务。
...
![The Swarm v1 architecture](graphics/image_01_003.jpg)
...
*   **Docker 堆栈**是创建多容器应用程序(由 Compose 描述)而不是单个容器的图像的二进制结果。
...
*   一个**发现服务**是一个由 Docker 提供的基于令牌的服务或者一个自托管的服务。对于自我托管的应用程序，您可以运行 HashiCorp Consul、CoreOS Etcd 或 Apache ZooKeeper 作为键值存储来充当发现服务。
...
![Docker for Mac](graphics/image_01_004.jpg)

如果你正在使用苹果操作系统 x，只需将 Docker 测试版图标拖放到你的应用程序文件夹中。输入你的测试版注册码，如果有的话，就完成了。

![Docker for Mac](graphics/image_01_005.jpg)

在 OS X，系统托盘中会有 Docker whale，您可以打开它并配置您的设置。Docker 主机将在您的桌面上本地运行。

![Docker for Mac](graphics/docker.jpg)
...
![Docker for Windows](graphics/image_01_007.jpg)

如果安装过程顺利，您将看到完成屏幕已准备好，可以启动窗口 Docker，如图所示:

![Docker for Windows](graphics/image_01_008.jpg)

在启动时，Docker 会将自己初始化为 Hyper-V。一旦这个过程完成，您就可以打开 PowerShell 并开始使用 Docker。

![Docker for Windows](graphics/image_01_009.jpg)

如果出现问题，您可以从托盘图标的菜单中打开日志窗口，并使用 Hyper-V 管理器进行检查。

![Docker for Windows](graphics/image_01_010.jpg)
...
![Swarm, yesterday](graphics/B05661_01_25.jpg)
...
![Create 4 cluster nodes with Docker Machine](graphics/image_01_012.jpg)

如果你再次查看机器列表，你会在图像名称旁边看到一个`*`，表示这是当前正在使用的机器。或者，您可以键入以下命令来打印当前活动的机器:
...
![Create 4 cluster nodes with Docker Machine](graphics/image_01_013.jpg)
...
![Create 4 cluster nodes with Docker Machine](graphics/image_01_014.jpg)
...
![Create 4 cluster nodes with Docker Machine](graphics/image_01_015.jpg)
...
![Configuring the Docker hosts](graphics/image_01_016.jpg)
...
要开始使用 Swarm v1(毫不奇怪)，您必须从 Docker 集线器中拉出`swarm`图像。打开四个终端，在第一个终端、源节点 0 ( `docker-machine env node0`)和第二个终端`node1`中为每台机器的环境变量添加源，并将`env`变量复制并粘贴到外壳中，以此类推，在完成上述更改标准端口和禁用 TLS 的步骤后，在每台终端上执行以下操作:
...
![Starting Docker Swarm](graphics/image_01_017.jpg)
...
![Starting Docker Swarm](graphics/image_01_018.jpg)
...
3.  Disable `DOCKER_CERT_PATH`.
    ...
    ![Starting Docker Swarm](graphics/image_01_019.jpg)

您应该有一个类似这样的配置:

![Starting Docker Swarm](graphics/image_01_020.jpg)

如果我们现在在`3376`连接到 Docker 群，并显示一些信息，我们会看到我们正在运行 swarm:

![Starting Docker Swarm](graphics/image_01_021.jpg)
...
![Starting Docker Swarm](graphics/image_01_022.jpg)
...
![Test your Swarm cluster](graphics/image_01_023.jpg)
...
![Test your Swarm cluster](graphics/image_01_024.jpg)
...
![Test your Swarm cluster](graphics/image_01_025.jpg)
