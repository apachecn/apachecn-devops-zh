...
如果我们在运行容器时没有指定显式的容器名称，那么 Docker 会自动为容器分配一个随机但唯一的名称。这个名字将由一位著名科学家的名字和一个形容词组成。这样的名字可以是`boring_borg`或`angry_goldberg`。我们的Docker很幽默，不是吗？
...
建造Docker引擎的地下室；我们也可以称之为**容器管道**，由两部分组成——**runc**和**容器**。
