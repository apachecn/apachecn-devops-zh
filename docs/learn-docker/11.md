...
该命令基于映像`fundamentalsofdocker/whoami:latest`创建名为`web`的服务，将容器港口`8000`发布到所有群节点上的港口`8000`，并为其分配秘密`api-secret-key`。
...
有时，我们想要将一个我们不能轻易改变或者不想改变的遗留应用容器化。这个遗留应用可能希望有一个秘密值作为环境变量。*我们现在要怎么处理？* Docker 将秘密作为文件呈现给我们，但是应用希望它们以环境变量的形式出现。
...
类似地，如果遗留应用期望秘密值作为条目出现在例如位于`/app/bin`文件夹中的名为`app.config`的 YAML 配置文件中，该文件的相关部分如下所示:
...
让我们做一个样本。假设我们有一个遗留应用在一个由映像`fundamentalsofdocker/whoami:latest`定义的容器中运行，该容器期望在应用文件夹中的文件`whoami.conf`中定义一个秘密`db_password`。我们可以在本地机器上定义一个文件`whoami.conf`，内容如下:
...
在容器内运行的应用能够在`/run/secrets/db-password`访问秘密。现在，SwarmKit 不允许我们在运行的服务中更新现有的秘密，因此我们必须首先删除这个秘密的过时版本，然后添加新的版本。让我们从以下命令开始删除:
...
*   *从环境变量到Docker的秘密[https://bit.ly/2GY3UUB](https://bit.ly/2GY3UUB)*