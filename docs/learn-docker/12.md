...
对于 Kubernetes 豆荚来说，情况就不同了。创建新的 pod 时，Kubernetes 首先创建一个所谓的暂停容器，其唯一目的是创建和管理 pod 将与所有容器共享的名称空间。除此之外，它没有任何用处，只是睡觉。**暂停**容器通过 **veth0** 连接至桥梁**码头 0** 。将成为 pod 一部分的任何后续容器都使用 Docker 引擎的特殊功能，该功能允许它重用现有的网络命名空间。这样做的语法如下:
