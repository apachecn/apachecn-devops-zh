...
![An Overview of Docker Swarm](graphics/image03_01.jpg)

### 注

要模拟一个 Docker Swarm 集群，我们需要在本地安装一个虚拟机管理程序(虚拟机管理程序类型 2 是作为软件应用程序安装在现有操作系统上的虚拟机管理程序)，在本例中是 VirtualBox，这将允许我们通过`docker-machine`在本地创建多个运行 Docker 的主机，然后将它们添加到 Swarm 集群中。部署到云供应商时，这是使用他们的计算服务实现的，例如 AWS 上的 EC2。
...
![Using Docker Engine to Create a Swarm](graphics/image03_03.jpg)

就在下面，我们有我们的命令列表:

![Using Docker Engine to Create a Swarm](graphics/image03_04.jpg)
...
![Creating Docker Machines](graphics/image03_05.jpg)

## 列出创建的机器

该命令将提供当前在您的主机上的所有 Docker 机器的列表和更多信息，例如机器的状态、驱动程序等:`docker-machine ls`

![Listing Created Machines](graphics/image03_06.jpg)
...
![Worker Machine Creation](graphics/image03_07.jpg)

![Worker Machine Creation](graphics/image03_08.jpg)

最后最后一个工作节点将显示如下:

![Worker Machine Creation](graphics/image03_09.jpg)

这样做之后，运行`docker-machine ls`，如果创建成功，您将看到类似如下的输出:

![Worker Machine Creation](graphics/image03_10.jpg)
...
![Initializing our Swarm](graphics/image03_11.jpg)
...
![Connecting to a Machine](graphics/image03_12.jpg)

### 注

在云供应商上使用`ssh protocol`将需要通过用户名和密码或`ssh keys`进行身份验证和/或授权。我们不会深入讨论这个问题，因为这是一个演示。
...
![Initializing Swarm Mode](graphics/image03_13.jpg)
...
![Adding Workers to our Swarm](graphics/image03_14.jpg)

然后，我们通过`ssh`连接到一个工作节点:

![Adding Workers to our Swarm](graphics/image03_15.jpg)

最后，我们将节点添加到集群中:

![Adding Workers to our Swarm](graphics/image03_16.jpg)
...
![Viewing a Cluster's Status](graphics/image03_17.jpg)
...
1.  `Ssh` into your first node:
    ...
    ![Activity 1 — Adding Nodes to a Cluster](graphics/image03_18.jpg)
...
2.  Run the invite command on the node to join the cluster. Remember, we got this command when we first initialized our manager node:
    ...
    ![Activity 1 — Adding Nodes to a Cluster](graphics/image03_19.jpg)

3.  Exit the node, `ssh` into another, and run the command:

    ![Activity 1 — Adding Nodes to a Cluster](graphics/image03_20.jpg)

4.  `Ssh` into the manager node to check the cluster status through `docker node ls:`

    ![Activity 1 — Adding Nodes to a Cluster](graphics/image03_21.jpg)
...
![Creating a Service](graphics/image03_22.jpg)

我们基于 WordPress 图像创建了一个网站，并将主机端口`80`映射到容器端口`80`。
...
![Listing Services](graphics/image03_23.jpg)
...
![Service Status](graphics/image03_24.jpg)
...
![How Do We Know Our Site is Running?](graphics/image03_25.jpg)

以下是 WordPress 在我们浏览器上的显示截图:

![How Do We Know Our Site is Running?](graphics/image03_26.jpg)
...
1.  Create a new node and name it `dbworker`:
    ...
    ![Activity 2 — Running Services on a Swarm](graphics/image03_27.jpg)

2.  Add the new worker to the swarm:

    ![Activity 2 — Running Services on a Swarm](graphics/image03_28.jpg)
...
3.  Create a new database service and name it `db`, using the postgres image as the base:
    ...
    ![Activity 2 — Running Services on a Swarm](graphics/image03_29.jpg)
...
4.  通过以下步骤验证`postgres`正在运行:
    1.  Map the `postgres` container running in `dbworker node` to your computer:
        ...
        ![Activity 2 — Running Services on a Swarm](graphics/image03_30.jpg)

    2.  Run `docker ps` to list running containers; this should have our `postgres` container and the status should be `UP`:

        ![Activity 2 — Running Services on a Swarm](graphics/image03_31.jpg)

    3.  Exit and stop the container through the following:

        ![Activity 2 — Running Services on a Swarm](graphics/image03_32.jpg)

# 向上和向下扩展服务

随着进入应用程序的请求数量的增加或减少，将需要扩展基础架构。我们最近使用了运行我们制作的相同 WordPress 安装的节点副本。

### 注

这是生产级设置的一个非常基本的例子。理想情况下，我们需要更多的管理器节点和副本，但是由于我们正在运行一个演示，这就足够了。

扩展包括根据应用程序的流量增加和减少资源。
...
![Scaling Our Database Service](graphics/image03_33.jpg)
...
![How Does Swarm Know Where to Schedule a Service?](graphics/image03_34.jpg)

缩小与放大非常相似，只是我们传递的副本数量比以前少。从以下输出中，我们缩小到一个复制副本，并验证复制副本数量为 1:

![How Does Swarm Know Where to Schedule a Service?](graphics/image03_35.jpg)

## 【Swarm 如何平衡副本之间的负载请求？

负载平衡器有助于处理和管理应用程序中的请求。在应用程序处理大量请求的情况下，可能在不到 5 分钟的时间内处理 1000 个请求，我们需要在应用程序上有多个副本和一个负载平衡器，特别是逻辑(后端)部分。负载平衡器有助于分发请求，防止实例过载，最终导致停机。
...
*   了解如何在 Dockerfiles 上创建应用程序模板
*   构建图像和容器，并将其推送到 Docker Hub
*   通过`docker-compose`管理的容器
*   了解了如何通过 Docker Swarm 编排我们的应用程序**