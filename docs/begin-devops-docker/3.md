...
Docker群是Docker容器的一个聚类工具。它允许你建立和管理一个集群的Docker**节点**作为一个单一**虚拟系统**。这意味着我们可以在计算机的多个主机上运行 Docker。
...
Docker机器通过`docker-machine cli.` Docker群作为服务运行来控制，该服务将所有Docker机器绑定在一起，并将它们统一在一个管理机器或节点下。这并不意味着集群中的机器在任何方面都是相同或相似的，它们都可能运行不同的服务或操作，例如，数据库主机和网络服务器。Docker Swarm 进来帮助编排主持人。
