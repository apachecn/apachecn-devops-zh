...
在上一课中，我们介绍了Docker和容器化。我们运行了示例 Python 和 JavaScript 脚本来演示如何将应用容器化以及如何构建映像。我们现在已经准备好运行一个超出这个范围的应用。
...
在我们总结这个概述时，重要的是要知道，随着时间的推移，开发人员创造了不同的堆栈变体来总结他们应用的前端、后端和数据库结构。以下是它们的列表及其含义(在本课程中，我们不会深入探讨):
...
第 4 行将指定`db` 服务容器将从其运行的映像。我们前面提到了一些堆栈；`db`映像可以是任何基于服务器的数据库。
