...
本课将讲述关于集装箱化的基本概念，作为我们稍后构建的映像和集装箱的基础。我们还将了解 Docker 如何以及为什么参与 DevOps 生态系统。在开始之前，我们将了解虚拟化与 Docker 中的容器化有何不同。
...
“有私人的 Docker 注册中心，重要的是你现在要意识到这一点。”？码头工人中心在 hub.docker.com。一些图片在[store.docker.com](http://store.docker.com)托管，但 Docker Store 包含官方图片。然而，它主要关注 Docker 映像应用商店的商业方面，并提供工作流供使用。
...
每个码头工人的形象都是从一个码头工人档案开始的。要创建应用或脚本的映像，只需创建一个名为 **Dockerfile** 的文件。
...
让你熟悉 Dockerfile 语法。本活动的目标是帮助理解和练习使用第三方映像和容器。这有助于更好地了解集装箱化对协作的影响。这通过构建已经存在的特性或资源来加快产品交付速度。
