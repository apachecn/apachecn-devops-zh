...
普罗米修斯通过Kubernetes服务发现了所有目标。它从每个服务中提取端口，并假设数据可通过`/metrics`端点获得。因此，我们在集群中的每一个可以通过 Kubernetes 服务访问的应用都被自动添加到普罗米修斯目标的`kubernetes-service-endpoints`组中。我们没有必要篡改普罗米修斯的配置来增加 T2。它刚刚被发现。很整洁，不是吗？
