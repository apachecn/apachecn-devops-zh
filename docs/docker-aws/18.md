...
7.  false–此命令仅用于创建堆栈。您应该根据需要使用`aws cloudformation deploy`命令来创建和更新堆栈。
...
1.  没错。
2.  倾听者。
3.  false–一旦创建了关联的应用程序负载平衡器侦听器，目标组就只能接受注册。
...
6.  您必须为用于加密秘密值的 KMS 密钥授予`kms:Decrypt`权限
...
1.  false–弹性豆茎支持单容器和多容器 Docker 应用程序
2.  `Dockerrun.aws.json`文件。
3.  没错。
4.  添加 IAM 权限，将 ECR 映像拉入您的弹性豆茎 EC2 实例使用的虚拟机实例角色。
5.  false–Elastic Beanstalk 对卷使用绑定装载，这将分配 root:root 权限，导致非根容器在写入卷时失败。
6.  false–您可以在`container_commands`键中将`leader_only`属性设置为 true，这样它只在一个弹性豆茎实例上运行命令。
7.  false–EB ssh 命令用于建立对弹性豆茎 EC2 实例的 SSH 访问。
8.  没错。
