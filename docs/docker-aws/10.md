...
移除动态端口映射的含义意味着，例如，todobackend 应用程序(运行在端口 8000 上)将在启用任务网络的情况下在端口`8000`上被外部访问，而不是通过动态映射的端口。这为生成大量网络流量的应用程序带来了更好的性能，也意味着您的安全规则可以针对应用程序运行的特定端口，而不是允许访问动态端口映射使用的短暂网络端口范围。
