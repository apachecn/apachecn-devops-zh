...
如果您现在导航回 ECS 控制台，并从左侧菜单中选择**资源**，您应该会看到一个名为`docker-in-aws/todobackend`的 ECR 存储库，如您的云信息堆栈中所定义的。如果您单击存储库，您将进入存储库详细信息页面，该页面为您提供存储库 URI、存储库中发布的映像列表、ECR 权限和生命周期策略设置。
