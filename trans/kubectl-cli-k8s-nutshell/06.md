...
哎呀，那里发生了什么？通过运行`$ kubectl get pods`命令，我们看到了一个`ErrImagePull`错误。让我们调查一下。在*第 1 章*、*介绍和安装 kubectl* 中，我们了解了`kubectl describe`命令；让我们用它来检查Pod 状态。要描述 PostgreSQL 窗格，请运行以下命令:
...
我们看到一个新的Pod `postgresql-56dcb95567-8rdmd`，它也正在崩溃。要检查此`postgresql`Pod ，运行以下命令:
...
嗯，这一次，`Events`没有列出太多关于为什么`postgresql`Pod 处于`CrashLoopBackOff`状态的信息，因为`bitnami/postgresql:10.13.0`图像被成功拉出。
