...
1.  Let's start by running the following command:
    ...
    我们使用了端口`80`，在该端口上，`nginx`服务暴露给了其他 Kubernetes 应用；`target-port=80`是我们的`nginx`集装箱港口。我们使用`port=80`容器是因为我们在 [*第 3 章*](03.html#_idTextAnchor039)*中部署的官方`nginx` Docker 映像([https://hub.docker.com/_/nginx](https://hub.docker.com/_/nginx))使用端口`80`。*
