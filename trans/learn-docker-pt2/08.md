...
显然，源容器的别名`ex`被解析为`172.17.0.3` IP 地址，并且接收者容器能够成功到达源。在安全容器通信的情况下，不允许在容器之间进行 ping 操作。我们将在[第 11 章](11.html)、*保护Docker集装箱*中看到关于保护集装箱方面的更多细节。
...
显然，两个`example_redis_1`和`example_web_1`容器已经启动并运行。集装箱名称以`example_`为前缀，为`docker-compose`项目名称。
