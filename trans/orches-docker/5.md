...
现在让我们做一些很酷的事情。对于不熟悉的人来说，Heroku 是一个云 PaaS，这意味着你在构建一个应用时所需要做的就是将它推送到 Heroku，它将被部署到 https://www.herokuapp.com 的 T2。您不需要担心应用如何或在哪里运行。只要 PaaS 支持您的技术堆栈，您就可以只在本地开发，并将应用推送到服务，使其在公共互联网上实时运行。
