...
这个是一个 `GET`请求 API。img/json`的请求将返回一个`JSON`响应，其中包含符合标准的图像的详细信息。
...
2.  如果响应的状态代码是 404，则表示图像不存在。尝试使img/create`参数拉取图像，返回步骤 1。
...
我们在[第 3 章](3.html "Chapter 3. Configuring Docker Containers")*配置Docker容器*的*链接容器*一节中学习了如何链接Docker。为了刷新你的记忆，这就是它的工作原理。
...
您可以在[第 3 章](3.html "Chapter 3. Configuring Docker Containers")、*配置Docker容器*中找到更多关于链接的信息。
...
Docker 容器从一个缩减的功能集开始。因此，您会发现您可以执行一些根操作，但不能执行其他操作。具体来说，非特权容器中的`root`用户不可能执行以下操作:
