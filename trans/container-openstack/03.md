...
OpenStack 是一款用于创建私有云和公共云的免费开源软件。它提供相互关联的组件集来管理和访问跨数据中心的大型计算、网络和存储资源池。用户可以使用基于网络的用户界面和命令行或者 REST APIs 来管理它。OpenStack 是由 Rackspace 和 NASA 在 2010 年开源的。目前，它由一个非盈利实体——OpenStack基金会管理。
...
中子是OpenStack的网络服务，它在OpenStack云中提供各种网络选项。它的旧名字是量子，后来改名为中子。中子使用大量插件来提供不同的网络配置。
