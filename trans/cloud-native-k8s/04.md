...
![Figure 4.1 – A basic control loop for a Kubernetes controller](Images/B14790_04_001.jpg)
...
复制集允许我们通知库本内特斯为特定的吊舱规格保持一定数量的吊舱。副本集的 YAML 与吊舱的非常相似。事实上，整个 Pod 规范嵌套在复制集 YAML 中的`template`键下。
...
现在，您应该了解了复制集规范的各个部分及其作用。让我们继续使用我们的复制集实际运行应用程序。
...
![Figure 4.2 – RollingUpdate process for a Deployment](Images/B14790_04_002.jpg)
...
![Figure 4.3 – DaemonSet spread across three nodes](Images/B14790_04_003.jpg)
...
这个 YAML 将创建一个包含我们的应用程序的五个副本的状态集。
...
有几种应用程序模式可以实现这一点，包括互斥锁和工作队列。此外，代码需要检查整个批处理任务的状态，这也可以通过更新数据库中的值来处理。一旦作业代码看到更大的任务完成，它应该退出。
...
![Figure 4.4 – Multi-tier application diagram](Images/B14790_04_004.jpg)
