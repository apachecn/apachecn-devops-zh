...
此外还有行业标准选项如**卡夫卡**和**拉比 tmq**可以使用社区Helm图和运营商部署在Kubernetes上，此外还有一些专门设计的开源和闭源选项:
...
这部作品没有太多其他令人兴奋的地方，所以让我们进入`initContainers`的开始。在`initContainers`和普通容器之间的这个Pod 中将有相当多的容器运行，所以我们将分别进行解释。接下来是第一个`initContainer`的例子:
