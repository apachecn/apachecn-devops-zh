...
对于日志记录，sidecar 容器可以从应用容器中获取应用日志(因为它们可以共享卷并在本地主机上通信)，然后将日志发送到集中的日志栈，或者解析日志以发出警报。监控也是类似的情况，边车Pod 可以跟踪和发送关于应用Pod 的指标。
...
如您所见，我们有一些基本的 NGINX 配置。重要的是，我们有`proxy_pass`字段，它将请求代理到`127.0.0.1`或本地主机上的一个端口。由于Pod 中的容器可以共享本地主机端口，这就充当了我们的边车代理。出于本书的目的，我们不会回顾所有其他行，但请查看 NGINX 文档，了解每一行的更多信息([https://nginx.org/en/docs/](https://nginx.org/en/docs/))。
