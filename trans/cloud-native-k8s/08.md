...
前两个效果`NoSchedule`和`NoExecute`提供了硬效果——也就是说，像节点选择器一样，只有两种可能性，要么在Pod 上存在容忍(我们稍后会看到)，要么Pod 没有被调度。`NoExecute`通过驱逐节点上所有有容忍能力的豆荚来增加这个基本功能，而`NoSchedule`让现有豆荚留在原地，同时防止任何没有容忍能力的新豆荚加入。
