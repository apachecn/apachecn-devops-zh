...
对我们非常有利的是，这些标准的基本映像仍然是现成的，并且可以在 Docker 注册表上公开获得；只需使用`docker search`和`docker pull`终端命令即可搜索和检索映像。这些将下载任何尚未位于 Docker 主机上的映像。Docker Registry 在提供官方基础映像方面变得越来越强大，人们可以直接使用这些映像，或者至少作为一个现成的起点来满足您的集装箱建造需求。
...
**试试看:**从码头的Docker登记处练习一个映像`pull`和`run`。
