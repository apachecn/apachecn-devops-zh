...
1.  Run the following **get** command, which will get us the resources and their statuses:
    ...
    ![Output displaying all the resources running in the default namespace](image/B17338_07_01.jpg)
...
2.  Focus your attention on the pods in your deployment. You can get the status of the pods with the following command:
    ...
    ![Checking the status of all the pods in the namespace](image/B17338_07_02.jpg)
...
![Getting additional details of all the pods in the namespace](image/B17338_07_03.jpg)
...
![Describing an individual pod to show a detailed output of that object](image/B17338_07_04.jpg)
...
![A flowchart describing the relationship between a deployment, a ReplicaSet, and pods](image/B17338_07_05.jpg)
...
![Running the kubectl get events command to display all events from the past hour](image/B17338_07_06.jpg)
...
2.  Type **/gb-frontend** and hit the *Enter* key to have your cursor brought to the image definition.
    ...
    ![Using Azure Cloud Shell to change the image tag from v4 to v_non_existent](image/B17338_07_07.jpg)
...
4.  Run the following command to list all the pods in the current namespace:
    ...
    ![Displaying all the pods in the current namespace to indicate status errors](image/B17338_07_08.jpg)
...
5.  Run the following command to get the full error details:
    ...
    ![Describing an individual pod to display more details on the error](image/B17338_07_09.jpg)

    图 7.9:使用描述显示了错误的更多细节

    这些事件清楚地表明，图像并不存在。诸如将无效凭证传递给私有 Docker 存储库之类的错误也会出现在这里。
...
![Creating entries in the guestbook application](image/B17338_07_10.jpg)
...
1.  You will use the **kubectl exec** command. This command lets you run commands on the command line of that pod. With the **-it** option, it attaches an interactive terminal to the pod and gives you a shell that you can run commands on. The following command launches a Bash terminal on the pod:
    ...
    ![Executing a command to launch and enter a Bash terminal on the pod](image/B17338_07_11.jpg)
...
4.  Add the following code at line 17, below the line **if ($_GET['cmd'] == 'set') {**. Remember, to edit a line in **vim**, you hit the *I* key. After you are done editing, you can exit by hitting *Esc*, and then type **:wq!** and press *Enter*:
    ...
    ![Output displaying the updated code that introduced an error and additional logging](image/B17338_07_12.jpg)
...
5.  You have now introduced an error where reading messages will work, but not writing them. You have done this by asking the front end to connect to the Redis master at the non-existent localhost server. The writes should fail. At the same time, to make this demo more visual, we added some additional logging to this section of the code.
    ...
    ![Guestbook application displaying entries from earlier](image/B17338_07_10.jpg)

    图 7.13:早期的条目仍然存在

6.  Now, create a new message by typing a message and hitting the **Submit** button:

    ![Creating a new message in the guestbook application](image/B17338_07_14.jpg)
...
![Refreshing the webpage to confirm there the new message wasn’t persisted](image/B17338_07_15.jpg)
...
2.  You will see entries such as those seen in *Figure 7.16*:
    ...
    ![Output displaying the new message as part of the application logs](image/B17338_07_16.jpg)
...
活跃度和就绪性探测不需要从应用程序中的同一个端点提供服务。如果您有一个智能应用程序，该应用程序可以在保持健康的情况下停止循环(意味着不再有流量发送到该应用程序)。为了实现这一点，它将使就绪探测失败，但使活动探测保持活动状态。
...
![Browsing to the external IP in the browser shows the application returning traffic from server 1 ](image/B17338_07_17.jpg)
...
![Output displaying a healthy application with its traffic load-balanced between server 1 and server 2](image/B17338_07_18.jpg)
...
![First, a command is executed to stop directing traffic to server 1\. Then, using kubectl get pods -w, the ready attribute of the server 1 pod changes from 1/1 to 0/1](image/B17338_07_19.jpg)
...
![Output displaying all traffic is directed to server 2](image/B17338_07_20.jpg)
...
![After restoring the readiness probe, traffic is load-balanced again](image/B17338_07_21.jpg)
...
![Output displaying a failing liveness probe that causes the pod to restart](image/B17338_07_22.jpg)
...
![More details on the pod showing the failing liveness probe caused the pod to be restarted](image/B17338_07_23.jpg)
...
![Running the kubectl get nodes command to get information about the nodes on the cluster](image/B17338_07_24.jpg)
...
![Adding the -o wide option to the command to display more details about the nodes](image/B17338_07_25.jpg)
...
![Output displaying CPU and memory utilization of the nodes](image/B17338_07_26.jpg)
...
![Output displaying requests and limits per pod, as well as the total of allocated resources](image/B17338_07_27.jpg)
...
1.  For this example, you will use the pods running in the **kube-system** namespace. Get all the pods in this namespace:
    ...
    ![Output displaying pods running in the kube-system namespace](image/B17338_07_28.jpg)
...
![Displaying the limits and requests for the CoreDNS pod](image/B17338_07_29.jpg)
...
![Running a command to get the actual pod consumption in all namespaces](image/B17338_07_30.jpg)
...
![An overview of the AKS Diagnostics pane in the Azure portal](image/B17338_07_31.jpg)
...
![Example output of Cluster Insights as seen in the Azure portal](image/B17338_07_32.jpg)

图 7.32:集群洞察的输出示例

AKS 诊断的网络部分允许您交互式地解决集群中的网络问题。当您打开“网络”视图时，会出现几个问题，这些问题会触发网络运行状况检查和配置审查。一旦您选择了其中一个选项，交互工具将给出这些检查的输出，如图 7.33*所示:*

![Diagnosing networking issues using AKS Diagnostics](image/B17338_07_33.jpg)
...
![Viewing CPU and memory utilization for the cluster in the Cluster tab](image/B17338_07_34.jpg)

图 7.34:集群选项卡显示了集群的 CPU 和内存利用率

群集指标还显示节点数和活动盒数。节点计数很重要，因为您可以跟踪是否有任何节点处于**未就绪**状态:

![Checking the node count and the number of active pods in the Cluster tab](image/B17338_07_35.jpg)
...
![An overview of the Reports tab that allows access to preconfigured monitoring workbooks](image/B17338_07_36.jpg)

图 7.36:报告选项卡允许您访问预配置的监控工作簿

例如，您可以浏览**部署**工作簿。这在*图 7.37* 中显示:

![An overview of the Deployments workbook, which shows the status of all the deployments](image/B17338_07_37.jpg)
 ...
 *![Drilling down in Log Analytics to get more details on the deployments](image/B17338_07_38.jpg)
...
![Detailed metrics of the nodes as seen in the Nodes pane](image/B17338_07_39.jpg)

图 7.39:节点窗格中节点的详细度量

请注意，可以从搜索栏旁边的下拉菜单中查看不同的指标。如果您需要更多的详细信息，您也可以点击并从您的节点获取 Kubernetes 事件日志:

![Clicking on the View Kubernetes event logs option to get the logs from a cluster](image/B17338_07_40.jpg)
...
![An overview of Log Analytics displaying a pre-created query to show the logs for your node](image/B17338_07_41.jpg)
...
![An overview of the Controllers tab displaying all the pods running in a ReplicaSet](image/B17338_07_42.jpg)
...
![An overview of the Containers tab displaying all the individual containers](image/B17338_07_43.jpg)
...
![Clicking the View container logs option to access the logs from the containers tab itself](image/B17338_07_44.jpg)

图 7.44:访问容器的日志

这将向您显示 Kubernetes 从您的应用程序中记录的所有日志。在本章的前面，您使用了 **kubectl** 来访问容器日志。使用这种方法可以提高工作效率，因为您可以在一个视图中编辑日志查询并关联来自不同 pod 和应用程序的日志:

![Displaying logs that Kubernetes logged from the application in a single view](image/B17338_07_45.jpg)

图 7.45:日志被收集并且可以被查询

除了日志，该视图还显示了为容器设置的环境变量。要查看环境变量，向下滚动**容器**视图的右单元格:

![Viewing the environment variables set for the container](image/B17338_07_46.jpg)
...
![An overview of the Deployments tab in the AKS Insights pane](image/B17338_07_47.jpg)
