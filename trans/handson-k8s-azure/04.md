...
如您所见，前端服务扩展到了六个Pod 。Kubernetes 还将这些豆荚分布在集群中的多个节点上。您可以使用以下命令查看正在运行的节点:
...
在本节中，您已经看到了用 Kubernetes 扩展豆荚是多么容易。此功能为您提供了一个非常强大的工具，不仅可以动态调整应用组件，还可以通过同时运行多个组件实例来为弹性应用提供故障转移功能。但是，您不会总是想要手动扩展您的应用。在下一节中，您将了解如何通过在部署中自动添加和删除 pods 来自动扩展应用。
...
2.  You can update the image tag with Helm and then watch the pods change using the following command:
    ...
    在新的Pod 上运行**描述**并为**图像**添加内容将向我们展示新的图像版本:
