...
在*容器图像*一节中的*第 1 章*、*容器和Kubernetes*介绍中介绍了容器图像和容器注册。容器映像包含启动实际运行的容器所需的所有软件。在本章中，您将构建包含您的函数的自定义容器图像。您需要一个地方来存储这些图像，以便 Kubernetes 可以提取它们并按比例运行容器。您将为此使用 ACR。ACR 是一个完全由 Azure 管理的私有容器注册表。
