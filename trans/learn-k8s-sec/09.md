...
`anchore-cli`能够与Kubernetes集群中的 Anchore Engine 进行对话。现在，让我们使用以下命令扫描映像:
...
虽然有许多不同的配置项/光盘管道和许多不同的映像扫描工具可供您选择，但概念是将映像扫描集成到配置项/光盘管道中可以保证Kubernetes工作负载以及Kubernetes集群。

## 构建阶段的扫描

有这么多CI/CD 工具，比如 Jenkins、Spinnaker、螺丝刀等供你使用。在本节中，我们将展示如何将映像扫描集成到 GitHub 工作流中。GitHub 中的工作流是包含多个作业的可配置自动化流程。这是一个类似于詹金斯管道的概念，但定义在 YAML 格式。映像扫描的简单工作流程就像定义一个触发器。通常在推送拉请求或提交时完成，建立构建环境，例如 Ubuntu。
...
在测试中，三个不同的Pod 将部署在 Kubernetes 集群中。其中一个有违反映像扫描策略的严重漏洞。因此，具有严重漏洞的工作负载被拒绝，如下所示:
