...
*   `kubelet` : `kubelet`在每个节点上运行。它向应用编程接口服务器注册节点。`kubelet`监控使用 Podspecs 创建的PODS，并确保PODS和容器健康。
...
容器储存是短暂的。如果容器崩溃或重新启动，它将从启动时的原始状态开始。Kubernetes 卷有助于解决这个问题。一个容器可以使用卷来存储一个状态。Kubernetes体积有一个荚的寿命；一旦PODS腐烂，体积也会被清理。一些支持的卷包括`awsElasticBlockStore`、`azureDisk`、`flocker`、`nfs`和`gitRepo`。
...
容器化使应用更加便携，因此锁定特定的云提供商变得不太可能。虽然有一些很棒的开源工具，比如`kubeadm`和`kops`，可以帮助 DevOps 创建 Kubernetes 集群，但是 Kubernetes 作为云提供商提供的服务听起来还是很有吸引力的。作为 Kubernetes 的最初创造者，谷歌从 2014 年开始向 Kubernetes 提供“T4”服务。叫做**谷歌 Kubernetes 引擎** ( **GKE** )。2017 年，微软提供了自己的 Kubernetes 服务，名为**Azure Kubernetes Service**(**AKS**)。AWS 在 2018 年提供**弹性Kubernetes服务** ( **EKS** )。
