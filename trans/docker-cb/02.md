...
在 Docker 1.6 中，增加了一个特性，这样我们就可以标记容器和映像，通过它们我们可以将任意键值对作为元数据附加。您可以将它们视为环境变量，这些变量对于在容器内运行的应用程序来说是不可用的，但是对于管理映像和容器的 Docker 客户端来说是可用的。附加到映像上的标签也会应用到开始使用这些映像的容器上。我们也可以在启动容器时给它们贴上标签。标记了映像或容器后，标签可用于过滤或选择目的。
