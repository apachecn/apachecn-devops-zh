...
Docker for Windows 开箱即用地支持 Windows 和 Linux 容器，在两者之间切换以构建您的多平台应用非常容易。它配备了Docker命令行界面客户端、Docker编写、Docker机器和Docker公证人。
...
*   默认情况下，Kubernetes 不运行。如果你想打开它，你需要右击任务栏中的 Docker whale 图标，然后选择设置。在“设置”菜单中，有一个 Kubernetes 选项卡。单击选项卡，然后单击启用Kubernetes选项并点击应用按钮:
