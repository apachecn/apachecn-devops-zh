...
![Figure 3.1 – The output from running helm search hub wordpress ](image/Figure_3.1.jpg)
...
![Figure 3.3 – A WordPress Helm chart from Helm Hub ](image/Figure_3.3.jpg)
...
![Figure 3.4 – The output for helm search repo –output yaml ](image/Image86715.jpg)
...
![Figure 3.5 – The output for helm search repo wordpress ](image/Figure_3.5.jpg)
...
![Figure 3.6 – The version lists for WordPress charts on the bitnami repository ](image/Figure_3.6.jpg)
...
![Figure 3.7 – The wordpress chart definition ](image/Figure_3.7.jpg)
...
![Figure 3.8 – The wordpress chart’s README file shown in the command line ](image/Figure_3.8.jpg)
...
安装 Helm 图表是一个简单的过程，可以从检查图表的值开始。在下一节中，我们将检查 WordPress 图表上可用的值，并描述如何创建允许自定义安装的文件。最后，我们将安装图表并访问 WordPress 应用程序。
...
![Figure 3.11 – The LoadBalancer value returned after running helm show values ](image/Figure_3.11.jpg)
...
该命令将使用`bitnami/wordpress`舵图安装一个名为`wordpress`的新版本。它将使用`wordpress-values.yaml`文件中定义的值来定制安装，图表将安装在`chapter3`命名空间中。它还将部署`8.1.0`版本，由`--version`标志定义。赫尔姆将安装没有这个标志的最新版本的赫尔姆图表。

如果图表安装成功，您应该会看到以下输出:

![Figure 3.12 – The output of a successful WordPress chart installation  ](image/Figure_3.12.jpg)
...
![Figure 3.13 – The output from the helm list command that lists the Helm releases ](image/Figure_3.13.jpg)
...
该注释表示在执行`test`子命令期间运行的钩子，我们将在 [*第 6 章*](06.html#_idTextAnchor292)*测试舵图*中更详细地探讨。这些测试挂钩为图表开发人员提供了一种机制，以确认图表是否按设计运行，并且可以被最终用户安全地忽略。
...
![Figure 3.14 – Setting type to NodePort ](image/Figure_3.14.jpg)

图 3.14–将`type`设置为`NodePort`

您还可以观察我们为 WordPress 用户设置的其他值。这些值在 WordPress 部署中被定义为环境变量，如图所示:

![Figure 3.15 – Values set as environment variables ](image/Figure_3.15.jpg)
...
![Figure 3.16 – ser-supplied values in the wordpress release ](image/Figure_3.16.jpg)
...
![Figure 3.17 – A subset of all the values for the wordpress release  ](image/Figure_3.17.jpg)
...
![Figure 3.18 – The wordpress deployment in the chapter3 namespace ](image/Figure_3.18.jpg)
...
在本书中，我们将把`--values`标志作为配置图表值的首选方法。原因是以这种方式配置多个值更容易。维护一个`values`文件还将允许我们将这些资产保存在一个**源代码管理** ( **单片机**)系统中，例如`git`，这使得安装更加容易复制。请注意，敏感值(如密码)永远不应存储在源代码管理存储库中。我们将在 [*第 9 章*](09.html#_idTextAnchor416)*掌舵安全注意事项*中介绍安全主题。暂时，重要的是记住不要将`secrets`推入源代码管理存储库中。当需要在图表中提供秘密时，推荐的方法是明确使用`--set`标志。
...
![Figure 3.19 – The WordPress blog page ](image/Figure_3.19.jpg)
...
![Figure 3.20: The WordPress admin console login page ](image/Figure_3.20.jpg)
...
![Figure 3.21 – The WordPress admin console page ](image/Figure_3.21.jpg)
...
![Figure 3.22 – The WordPress profile page ](image/Figure_3.22.jpg)
...
![Figure 3.23 – replicaCount in the helm show values command ](image/Figure_3.23.png)
...
![Figure 3.24 – The values under the resources stanza ](image/Figure_3.24.jpg)
...
![Figure 3.25 – The output for helm upgrade ](image/Figure_3.25.jpg)
...
2.  Run the `helm get values` command to inspect the values used in the upgrade:
    ...
    ![Figure 3.26 – The output of the helm get values ](image/Figure_3.26.jpg)
...
![Figure 3.27 – The output for replicaCount ](image/Figure_3.27.jpg)
...
这些秘密中的每一个都对应于版本修订历史的一个条目，可以通过运行`helm history`命令来查看:
...
![Figure 3.28 – The output of checking a specific revision ](image/Figure_3.28.jpg)
...
![Figure 3.29 – The values from the latest stable release ](image/Figure_3.29.jpg)
...
如果您已经成功地回滚了 WordPress 发行版，您就接近本章练习的结尾了。最后一步是通过利用`uninstall`子命令将 WordPress 应用程序从 Kubernetes 集群中移除，我们将在下一节对此进行描述。
...
![Figure 3.30 – Output showing PersistentVolumeClaim ](image/Figure_3.30.jpg)
