...
前面的输出显示了WordPress 图表值的开始。其中许多属性已经设置了默认值，这意味着如果这些值没有被覆盖，它们将代表图表的配置方式。例如，如果`image`值没有在`values`文件中被覆盖，WordPress 图表使用的映像将使用来自 docker.io 注册表的`bitnami/wordpress`容器映像作为`5.3.2-debian-9-r0`标签。
