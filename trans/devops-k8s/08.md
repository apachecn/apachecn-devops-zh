...
在本例中，我们通过`roleRef`将`Role`与用户绑定。库本内特斯支持不同种类的`roleRef`；我们可以在这里替换从`Role`到`ClusterRole`的那种:
...
污点和容忍被用来阻止一组吊舱在某些节点上调度运行。污点被应用于节点，而容忍被指定给荚。污点的值可能是`NoSchedule`或`NoExecute`。如果运行一个受污染节点的单元没有匹配的容忍度，这些单元将被驱逐。
