...
具有持久卷的状态集需要动态资源调配和`StorageClass`，因为状态集可以扩展。Kubernetes 需要知道在添加更多PODS时如何配置持久卷。
...
Kubernetes 有**资源 QoS** ( **服务质量**)的概念，帮助管理员根据不同的优先级分配和管理PODS。根据 pod 的设置，Kubernetes 将每个 pod 分类为:
...
如果我们再创建一个资源请求内存为 500 MB 的 pod，那么 Kubernetes 调度程序会将这个 pod 分配给节点 2。但是，如果 pod 没有资源请求，结果将会因节点 1 或节点 2 而异。因为 Kubernetes 不知道这个PODS会消耗多少内存:
